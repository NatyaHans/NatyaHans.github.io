...
sub run_blast{
    ...
    my @splitname = split /\./,$inputdb;
    my $splitted = $splitname[0];
    
    my $outfile = $rh_opts-> {'name'}.".".$splitted.'.blastn';
    my $dbfile = $rh_opts-> {'name'}.".".$splitted.'.db.fasta';
    my $gene_file = $rh_opts->{'genes'}; 
   # my $cmd = "blastp -outfmt 6 -query $gene_file -db $dbfile -out $outfile";  
    my $cmd = "blastn -outfmt 6 -query $gene_file -db $dbfile -max_target_seqs 5000 -out $outfile";  
    #print "$cmd\n"; 
    system($cmd);
}
...
sub make_blastdb {
    ...
    my @splitname = split /\./,$inputdb;
    my $splitted = $splitname[0];
    my $outfile = $rh_opts->{'name'}.".".$splitted.".db.fasta";
    my $null_file = $rh_opts->{'name'}.".".$splitted.".null" ;
