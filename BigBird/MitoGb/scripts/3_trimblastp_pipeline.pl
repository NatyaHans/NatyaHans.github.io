...
################################################################################################BLASTP 
    my $blast_file = $rh_opts-> {'name'} . ".blastp";  
...
#print "Hash 7 Natya family hash that stores the ti and taxa";
...
# to match the trimmed seq and gi with gi and taxa hash (table hash)
    ...
    my $trimmed_output = $rh_opts->{'name'}.'.trimmed.fasta';  #open output file for trimmed sequences
    open OUTTRIMMED, ">$trimmed_output" or die "cannot open $trimmed_output:$!";
    my $taxlist= $rh_opts->{'name'}.'.taxlist';
