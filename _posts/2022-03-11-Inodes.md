...
```
The output would be multiple lines and each line has two columns:
column 1: the first should be the inode count
column 2: the second should be the path
```
...
====
Let me further break down the piped commands.
find command looks for files in the path "FOLDERPATH" provided by the user
